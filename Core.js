...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    Hello ${pushname} dear 👋. I am ${global.BotName}, a bot developed by: Gappu to take your WhatsApp usage into next level.
    ...
    ©️ *${global.BotName}* All Rights Reserved by: *Gappu*
    `
...
case 'support': case 'supportgc':
    ...
    reply(`*My developer's group:* https://instagram.com/_g.a.p.p.u_`)
    break

case 'repo': case 'botrepo':
    
    reply(`*My Source Code:* https://instagram.com/_g.a.p.p.u_`)
    break
...
case 'swm': case 'take': case 'stickerwm': case 'steal':{
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
if (!args.join(" ")) return reply(`Use command: -steal Miku|By: Gappu`)
const swn = args.join(" ")
...
case 'handsomecheck':
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
				if (!text) return replay(`Tag Someone, Example : ${prefix + command} @Gappu`)
...
case 'charactercheck':
 ...
 case 'dare':
    ...
    if (isBanChat) return reply(mess.bangc)
         ...
         "say i love the bot owner Gappu through voice note",
...
case 'truth':
    ...
    if (isBanChat) return reply(mess.bangc)
                 ...
                 "do you love the bot creator Gappu?",
...
case 'help': case 'h': case 'menu': case 'allmenu': case 'listmenu':{
 ...
 const helpmenu = `Hello *${pushname}* buddy,

I am *Miku*, a bot developed by *Gappu*.
...
case 'add':{     			
 ...
 case "tts":  case "texttospeech":  case "say": case "speak":{
    ...
    var replqr =  await getBuffer(`https://miku-qr--Gappu.repl.co/`)
                               var qrbutton = [
